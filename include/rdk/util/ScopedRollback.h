...
class ScopedRollback {
  public:
     ...
     * @param function_to_rollback Function to rollback, if not committed.
     */
    void add(std::function<void()>&& function_to_rollback) {
        rollback_functions_.push_back(std::move(function_to_rollback));
    }
